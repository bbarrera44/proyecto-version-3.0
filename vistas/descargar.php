...
$documento=$_COOKIE['documento'];
	...
	$conexion=new PDO("mysql:hos=localhost;dbname=proyecto","root","") or die (mysqli_error($conexion));

	$registro=$conexion->prepare("SELECT * from aprendices inner join fichas on aprendices.ficha=fichas.ficha where documento='$documento'");
	$registro->execute();

foreach ($registro as $row) {

	if ($row['n_programa']=='ADSI') $row['n_programa']='ANALISIS Y DESARROLLO DE SISTEMAS DE INFORMACION';
	if ($row['n_programa']=='TEA') $row['n_programa']='TECNICO EN ELABORACION DE ANIMACIONES';
	if ($row['n_programa']=='DIM') $row['n_programa']='DISENO E INTEGRACION DE MULTIMEDIA';
	if ($row['n_programa']=='TPS') $row['n_programa']='TECNICO EN PROGRAMACION DE SOFTWARE';
	...
	$pdf->Cell(190/*Pixeles de la linea*/,10/*Alto de pixeles*/,utf8_decode('ACTO ACADÉMICO No. '.date("Y").'')/*Contenido*/, 0 /*Borde del texto 1=Si 0=No*/,1/*Salto de linea*/,'C'/*Alineacion*/);
	$pdf->SetX(60);
	$pdf->MultiCell(90,6,utf8_decode('POR EL CUAL SE CANCELA EL rowISTRO DE MATRICULA POR DESERCIÓN'),0,'C',0);
	$pdf->Ln();
	$pdf->Cell(190,6,utf8_decode('PROGRAMA'),0,1,'C');
	$pdf->Cell(190,7,utf8_decode($row['n_programa']),0,1,'C');
	$pdf->Cell(190,10,utf8_decode('FICHA '.$row['ficha']),0,1,'C');
	$pdf->MultiCell(190,6,utf8_decode('LA SUBDIRECTORA DEL CENTRO DE ELECTRICIDAD, ELECTRÓNICA Y TELECOMUNICACIONES DE LA rowIONAL DISTRITO CAPITAL DEL SERVICIO NACIONAL DE APRENDIZAJE SENA'),0,'C',0);
	$pdf->Ln();

	$pdf->SetFont('Arial','','12');$pdf->SetX(20);
	$pdf->MultiCell(170,5,utf8_decode('En ejercicio de las funciones delegadas mediante Acuerdo No. 0007 de 2012, con Resolución de Nombramiento No. 00034 de 13 de Enero de 2017 y Acta de Posesión No. 017 del 13 de Enero de 2017, procederá a tomar una decisión conforme a lo señalado en el Capítulo VII, Artículo 22, Numeral 4 del rowlamento del Aprendiz, en consideración a los siguientes:'),0,'J',0);
	$pdf->Ln();

	$pdf->SetFont('Arial','B','12'); 
	$pdf->Cell(190,10,utf8_decode('1. HECHOS'),0,1,'C');
	$pdf->SetFont('Arial','','12');$pdf->SetX(20);
	$pdf->MultiCell(170,5,utf8_decode('El Instructor responsable del seguimiento, previa verificación exhaustiva, reportó al Coordinador Académico que el siguiente aprendiz se encuentran incursos en la causal de deserción, de acuerdo a lo señalado en el Capítulo VII, Artículo 22, Numeral 4 del rowlamento del Aprendiz.'),0,'J',0);
	$pdf->Ln();$pdf->Ln();

	$header=array('Nombres','Apellidos','Tipo de Documento','N° de Documento');

	$pdf->SetX(20);
	$pdf->TablaBasica($header,$row['nombres_aprendiz'],$row['apellidos_aprendiz'],$row['tipo_documento'],$row['documento']);
	$pdf->Ln();$pdf->Ln();

	$pdf->SetFont('Arial','B','12'); 
	$pdf->Cell(190,10,utf8_decode('2. DESCARGOS'),0,1,'C');
	$pdf->SetFont('Arial','','12');$pdf->SetX(20);
	$pdf->MultiCell(170,5,utf8_decode('El aprendiz, de acuerdo en lo establecido en el Capítulo VII, Artículo 22, Numeral 4, Parágrafo del rowlamento del Aprendiz, conto con cinco (5) días hábiles para aportar las evidencias que justificaran su incumplimiento; pero él no dio respuesta ni se presentó; es decir, no hizo los descargos correspondientes.'),0,'J',0);
	$pdf->Ln();
	...
	$pdf->SetFont('Arial','','12');$pdf->SetX(20);
	$pdf->Cell(170,10,utf8_decode('Norma infringida en el rowlamento del Aprendiz Capitulo VII, Artículo 22, Numeral 4:'),0,1,'J');
	...
	$pdf->SetFont('Arial','','12');$pdf->SetX(20);
	$pdf->MultiCell(170,5,utf8_decode('Es de resaltar que, al ingreso al Centro de Electricidad, Electrónica y Telecomunicaciones de la rowional Distrito Capital SENA, los aprendices son conocedores de las rowlas de comportamiento, el deber de cumplir con las actividades y compromisos que adquieren e igualmente son conscientes que el no cumplimiento de estos y la no justificación en caso de infringirlos acarrea sanciones; además los aprendices tienen el compromiso de cumplir y hacer cumplir el rowlamento del Aprendiz SENA. Con fundamento en las pruebas allegadas y los hechos, una vez analizados en conjunto, se desprende que la conducta del (os) aprendiz (ces) es considerada deserción de acuerdo con lo señalado en el Capítulo VII, Artículo 22, Numeral 4 del rowlamento del Aprendiz.'),0,'J',0);
	$pdf->Ln();
	...
	$pdf->SetFont('Arial','','12');
	$pdf->MultiCell(145,5,utf8_decode('SANCIONAR al aprendiz relacionado por el término de seis (6) meses de conformidad con lo establecido en el Capítulo VII, Numeral 4, Parágrafo, del rowlamento del Aprendiz.'),0,'J',0);
	$pdf->Ln();
	...
	$pdf->Cell(190,5,utf8_decode('rowional Distrito Capital'),0,1,'C');
	$pdf->Ln();
	$pdf->SetX(20);
	$pdf->MultiCell(170,5,utf8_decode('La decisión del Subdirector se notifica el día '.date("d").' de '.date("m").' del año '.date("Y").' a: '.$row['nombres_aprendiz'].$row['apellidos_aprendiz'].', identificada con la '.$row['tipo_documento'].' No. '.$row['documento'].', Ficha No. '.$row['ficha'].', del programa '.$row['n_programa'].'.'),0,'J',0);
	$pdf->Ln();
	$pdf->Ln();
	$pdf->Ln();
